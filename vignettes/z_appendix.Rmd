...
- `data()` (for package provided data)
- `load()` (for saved objects)
- `loadhistory()` 
- `read()` methods (ie. readLines, readRDS, read.csv...)
- [Bioconductor specific](https://contributions.bioconductor.org/important-bioconductor-package-development-features.html#commonimport)
- User created/defined

## Export/Write/Save

- `write()`\`write.csv` lots of options to customize writting a table
- `save()` saves objects
- `save.image()` saves workspace
- `savehistory()`
- `png()`\`pdf()`...with `dev.off()` or `dev.print()`
- `ggsave()`
- User created/defined
...
```{r}
base_url <-
    "https://raw.githubusercontent.com/lshep/RPG520/main/inst/extdata/"
...
If you have already installed the package on your system, the data included in
the workshop will already be on your system as package data. Package's that
include data in `inst/extadata` will have the data available in the installation
path. You can return the path to the local file on your system using `system.file`:

```{r, eval=FALSE}
path <- system.file(package = "RPG520", "extdata", "ALL.csv")
all <- readr::read_csv(path)
```


## Finding New Data

### Website, Databases, Github repos

There are lots of places to download and find data from the internet. Make sure
to read any licensing to ensure correct usage.

#### In Packages

Packages can directly or indirectly provide data to users.

- Data directly provided are in the source directory `data`. They must provide a
  help page accessible with `?` and can be loaded by using the `data()` function. 

- Indirect Data is provided in the source directory `inst` (typically)
  `inst/extdata`. The path to the data can be accessed with
  `system.file(package="nameOfPackage", "extdata", "nameOfFile")`. An
  appropriate load or read function can then by used to load data into R
  session.
...
Bioconductor consists of four types of packages: Software, Experiment Data,
Annotation Data, and Workflows.

[Software Packages][] typically provide functions for data analysis. The package
itself could include small datasets. Data included in the data directory will be documented
and available via `data` function in R. Data included in the
`inst/extdata` will be available by using `system.file` and using appropriate
load or read functions in R for the file type. Software packages themselves may
also be direct interfaces with web based clients. See some examples of [web
clients][] in Bioconductor.

[Experiment Data Packages][] packages contain curated data from an
experiment, teaching course or publication and in most cases contain a single
data set, or provide an interface to interact and download from an internet
resource.

[Annotation Packages][] are database-like packages that provide
information linking identifies (e.g., Entrez gene names or Affymetrix probe ids)
to other information (e.g., chromosomal location, Gene Ontology category).

The last type of package, [Workflows][], generally do not contain R code but may
contain data or links to data. They consist of in depth or end-to-end analysis
of a type of data. 

[Software Packages]: https://bioconductor.org/packages/release/BiocViews.html#___Software
[web clients]: https://bioconductor.org/packages/release/BiocViews.html#___ThirdPartyClient
[Experiment Data Packages]: https://bioconductor.org/packages/release/BiocViews.html#___ExperimentData
[Annotation Packages]: https://bioconductor.org/packages/release/BiocViews.html#___AnnotationData
[Workflows]: https://bioconductor.org/packages/release/BiocViews.html#___Workflow

#### Hubs

Bioconductor has a concepts of Hubs, namely: [ExperimentHub][] and
[AnnotationHub][]. The Hub itself is a database with pointers to download a
resource. Hub resources can be community contributed or provided by the
...
[AnnotationHub]: https://www.bioconductor.org/packages/AnnotationHub/

See previous
[presentation](https://docs.google.com/presentation/d/10kG9G_mJLR4paNZ7JS-BFEKaZEFJxYFsznPuxtUnzug/edit#slide=id.p)
...
button on the markdown document pane.

Cheatsheets:

- [rmarkdown-cheatsheet V1](https://rstudio.github.io/cheatsheets/html/rmarkdown.html)
- [rmarkdown-cheatsheet V2](https://rmarkdown.rstudio.com/lesson-15.HTML)
- [rmarkdown-cheatsheet V3](https://www.markdownguide.org/cheat-sheet/)
...
- [biocthis][] extends usethis with more Bioconductor compatibility.
